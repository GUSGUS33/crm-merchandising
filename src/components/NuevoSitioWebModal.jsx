...
const NuevoSitioWebModal = ({ isOpen, onClose, onSitioCreated }) => {
  ...
  const handleInputChange = (e) => {
    ...
    // Auto-generar sitio_id basado en el nombre
    if (name === 'nombre' && value) {
      const autoId = 'SW' + value.replace(/[^a-zA-Z0-9]/g, '').substring(0, 3).toUpperCase().padEnd(3, '0')
      setFormData(prev => ({
        ...prev,
        sitio_id: autoId
      }))
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const { data, error } = await db.createSitioWeb(formData)
      
      if (error) {
        console.error('Error al crear sitio web:', error)
        alert('Error al crear el sitio web. Por favor, inténtalo de nuevo.')
        return
      }

      // Notificar al componente padre
      if (onSitioCreated && data && data[0]) {
        onSitioCreated(data[0])
      }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Nuevo Sitio Web</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
...
export default NuevoSitioWebModal
